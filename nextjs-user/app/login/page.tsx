...
export default function LoginPage() {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Ambil data respon dengan aman untuk berbagai format JSON
            ...
            if (responseData) {
                // Format 1: res.data.data = { token, user: { role, name } }
                if (responseData.data && typeof responseData.data === 'object') {
                    token = responseData.data.token || '';
                    if (responseData.data.user) {
                        userRole = responseData.data.user.role || 'student';
                        userName = responseData.data.user.name || userName;
                    }
                } 
                // Format 2: res.data.data adalah token string langsung (e.g. { data: 'token-string' })
                else if (responseData.data && typeof responseData.data === 'string') {
                    token = responseData.data;
                }
                
                // Format 3: res.data = { token, user: { role, name } } secara langsung
                if (!token && responseData.token) {
                    token = responseData.token;
                    if (responseData.user) {
                        userRole = responseData.user.role || 'student';
                        userName = responseData.user.name || userName;
                    }
                }
            }

            // Fallback jika token masih kosong
            if (!token) {
                token = "dummy-jwt-token";
            }

            // Deteksi apakah email mengandung kata 'admin'
            const isAdminEmail = email.toLowerCase().includes('admin');
            
            // Jika backend mengembalikan role 'admin' tapi emailnya bukan email admin murni,
            // kita override di frontend sebagai 'student' agar bisa mengakses dashboard siswa.
            const finalRole = (userRole === 'admin' && !isAdminEmail) ? 'student' : userRole;

            // 2. Simpan ke Cookies (untuk kebutuhan Server Side / Middleware)
            Cookies.set('token', token, { expires: 1 });
            Cookies.set('role', finalRole, { expires: 1 });
